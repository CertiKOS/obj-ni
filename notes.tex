...
\begin{definition}[Event in Multi-Object Setting]
Given a coherence space generated by the object signature $O$, an event is a tuple of the owner object (principal) and the corresponding token drawn from the web \abs{O}.
\end{definition}
...
\begin{example}[Event Calendar] \label{ex:calendar}
  Let \mycode{Calendar} and $S$ be the calendar and scheduler objects with the following signatures:
  \begin{align*}
    \mycode{Calendar} = & \{ \\
