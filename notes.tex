...
If $t_1 = \mycode{putSalary}(1, 2000) \tracecat \mycode{putSalary}(1, 3000)$ and  $t_2 = \mycode{putSalary}(1, 4000) \tracecat \mycode{putSalary}(1, 1000)$, then \treq{t_1}{t_2}{L}{\classpart}.
...
\medskip
%%Definition~\ref{def:relaxni} can admit declassification events. For example,
%% $m().v_1=\mycode{avgSalary}().5000$ and  $m().v_2 = \mycode{avgSalary}().6000$, then \treq{t_1  \tracecat  m(\vec{v}).v_1}{t_2 \tracecat  m(\vec{v}).v_2}{L}{\classpart} satisfying Definition~\ref{def:relaxni}.
